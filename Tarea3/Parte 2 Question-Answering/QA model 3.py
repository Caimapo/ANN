...
encoder = SimpleRNN(hidden_dim, return_sequences=True)(embedded)
...
decoder = SimpleRNN(hidden_dim, return_sequences=True)(sent_representation)
...
plot_model(model, to_file=os.path.join(os.getcwd(),'results','model_RNN.png'))
...
model.save(os.path.join(os.getcwd(),'results','model_RNN_.h5'))

with open(os.path.join(os.getcwd(),'results','model_RNN_history.pickle'), 'wb') as r_file:
    pickle.dump(model_history['history'].history,r_file)
with open(os.path.join(os.getcwd(),'results','model_RNN_time.pickle'), 'wb') as r_file:
