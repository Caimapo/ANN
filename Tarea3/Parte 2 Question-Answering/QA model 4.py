...
encoder = Bidirectional(CuDNNLSTM(hidden_dim, return_sequences=True))(embedded)
...
plot_model(model, to_file=os.path.join(os.getcwd(),'results','model_Bidirectional.png'))
...
model.save(os.path.join(os.getcwd(),'results','model_Bidirectional_.h5'))

with open(os.path.join(os.getcwd(),'results','model_Bidirectional_history.pickle'), 'wb') as r_file:
    pickle.dump(model_history['history'].history,r_file)
with open(os.path.join(os.getcwd(),'results','model_Bidirectional_time.pickle'), 'wb') as r_file:
